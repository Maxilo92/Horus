...
void TrackingSystem::trackingWorkerLoop() {
    ...
    while (m_running) {
...

        {
            ...
            trackingFrame = m_trackingFrameCopy; // Shallow copy
        ...
        UICommandState cmd = m_blackboard.consumeUICommand();
        ...
        try {
            // --- 1. Handle external lock/unlock commands ---
            ...
            // --- 2. Pixel lock acquisition ---
            if (cmd.pixelLockRequested) {
                cv::Rect tr = cmd.pixelLockRect;
                int x1 = std::max(0, std::min(tr.x, trackingFrame.cols - 1));
                int y1 = std::max(0, std::min(tr.y, trackingFrame.rows - 1));
                int x2 = std::max(0, std::min(tr.x + tr.width,  trackingFrame.cols));
                int y2 = std::max(0, std::min(tr.y + tr.height, trackingFrame.rows));
            ...
            // --- 3. Pixel lock rect update (user resized the selection box) ---
            if (cmd.pixelLockRectUpdateRequested) {
                cv::Rect rect = cmd.pixelLockRect;
                int ux1 = std::max(0, std::min(rect.x, trackingFrame.cols - 1));
                int uy1 = std::max(0, std::min(rect.y, trackingFrame.rows - 1));
                int ux2 = std::max(0, std::min(rect.x + rect.width,  trackingFrame.cols));
                int uy2 = std::max(0, std::min(rect.y + rect.height, trackingFrame.rows));
                rect = cv::Rect(ux1, uy1, ux2 - ux1, uy2 - uy1);
            ...
            if (settings.enableTracking) {
                // --- 4. High-sensitivity re-acquisition ---
                    ...
                    for (auto& obj : tracked) {
                        if (obj.class_id == 0 && obj.is_confirmed) {
                            bool runRecognition = false;
                            {
                                ...
                                runRecognition = (it == m_trackIdToFace.end());
                            }
                            if (runRecognition) {
                                auto faceResults = m_faceRecognizer->process(
                                    trackingFrame, obj.box, settings.faceRecognitionThreshold);
                                if (!faceResults.empty() && faceResults[0].identityId != -1) {
                                    std::lock_guard<std::mutex> lock(m_faceMutex);
                                    m_trackIdToFace[obj.track_id] = {
                                        faceResults[0].identityId, faceResults[0].name};
                                }
                            }
                            {
                                std::lock_guard<std::mutex> lock(m_faceMutex);
                ...
                if (m_pixelLockActive && m_sharedLockedTarget.state != TrackingState::SEARCHING) {
                    if (!cmd.pixelLockDragging) {
                        ...
                        int pad   = 80;
                        int rectW = predicted.width  + pad * 2;
                        int rectH = predicted.height + pad * 2;
                        int rectX = predicted.x - pad;
                        int rectY = predicted.y - pad;

                        int sx1 = std::max(0, std::min(rectX, trackingFrame.cols - 1));
                        int sy1 = std::max(0, std::min(rectY, trackingFrame.rows - 1));
                        int sx2 = std::max(0, std::min(rectX + rectW,  trackingFrame.cols));
                        int sy2 = std::max(0, std::min(rectY + rectH, trackingFrame.rows));
                        ...
                        if (searchRect.width >= m_pixelTemplate.cols * 0.95f &&
                            ...
                            std::vector<float> scales = {0.95f, 1.0f, 1.05f};

                            for (float scale : scales) {
                                cv::Mat scaledTemplate;
                                if (std::abs(scale - 1.0f) > 1e-3) {
                                ...
                                if (searchRect.width >= scaledTemplate.cols &&
                                    searchRect.height >= scaledTemplate.rows) {
                                    cv::Mat result;
                                    cv::matchTemplate(trackingFrame(searchRect), scaledTemplate,
                            ...
                            if (bestMaxVal > 0.5) {
                                ...
                                if (bestMaxVal > 0.85) {
                                    cv::Mat patch;
                                    cv::getRectSubPix(trackingFrame, m_pixelTemplate.size(), 
                ...
                // --- 10. YOLO-based locked target state update ---
                if (!m_pixelLockActive && m_sharedLockedTarget.state != TrackingState::SEARCHING) {
                    bool found = false;
                ...
                // --- 11. Audio: lock state transitions + continuous pulse ---
                {
                    ...
                    if (m_prevLockState != TrackingState::LOCKED && cur == TrackingState::LOCKED) {
                        ...
                        // Update pulse strength each frame:
                        // combine detection confidence with a stability bonus that
                        // grows as the lock holds without losing the target.
                        ...
                        // Stability bonus: up to +0.15 after ~5s of clean lock at 30fps
                ...
                // Expose pixel target as a tracked object for the render thread
                if (m_pixelLockActive && m_sharedLockedTarget.state != TrackingState::SEARCHING) {
                    TrackedObject po;
                ...
                if (settings.dataLoggingEnabled && m_dataLogger.isOpen()) {
                    ...
                    for (const auto& mt : motionTracks) {
                        ...
                        for (const auto& obj : tracked) {
                            ...
                            if (inter.area() > 0 && (double)inter.area() / mt.box.area() > 0.2) {
                                overlaps = true; break;
                            }
                        }
                        if (!overlaps) {
        ...
        perf.dataLoggerQueue = static_cast<int>(m_dataLogger.getQueueSize());
